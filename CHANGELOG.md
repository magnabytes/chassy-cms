...
User-facing release notes for Chassy CMS. Ships in the deploy zip alongside the app.

This file only gets a new entry when an official release goes out — not after every dev session. Entries are written in plain language for a site owner, not a developer: what's new, what's fixed, what (if anything) they need to do. No file paths, function names, or internal implementation detail.
